...
// -----------------------------------------------------------------------------
	...
	var serverSpec = function (server_id, nservers) {
		...
		var pos = paxos.getServerIndexById(state.current, server_id);
	...
	//No need for votes. I'll use terms as paxos instance id
			...
			if (!serversSame) {
				...
				if (server.state == 'candidate') {
					state.current.servers.forEach(function (peer) {
						...
						var coord = util.circleCoord(paxos.getServerIndexById(this.model, peer.id) / this.nsrv,
